...
- Node type/size: from tfvars or `.env` (`NODE_INSTANCE_TYPES`, `NODE_MIN_SIZE`, `NODE_DESIRED_SIZE`, `NODE_MAX_SIZE` default to `t3.xlarge` 2/2/4; list supports multiple instance types)
...
1) **Bootstrap remote state** (local or Jenkins):
   ```bash
   ./scripts/load_env.sh            # loads .env and writes terraform/bootstrap/override.auto.tfvars
